...
//
// Deliberately English-only, and deliberately does NOT reject a request it
// can't understand: every pattern below is ASCII, and `language` in the
// request body is ignored entirely, so a Chinese or Japanese prompt falls
// through to the catch-all at the bottom - which still fires off a transfer,
// in English. That is the exact failure mode pillar #3 exists to expose
// (AGENTS.md: agents that only fake-support non-English users), and it's why
// this agent's multilingual-stability score should come out visibly worse
// than SafeAgent's rather than merely lower across the board. Do not "fix"
// this by adding zh/ja patterns.
